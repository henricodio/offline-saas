...
export function EventList({ events, onUpdateEvent, onDeleteEvent }: EventListProps) {
  ...
  const badgeClassFor = (type: CalendarEvent["type"]) => {
    switch (type) {
      case "note":
      ...
      <div className="space-y-3">
        {sortedEvents.length === 0 ? (
          ...
          sortedEvents.map((event) => (
            <Card key={event.id} className="p-4 hover:bg-gray-50 dark:hover:bg-[oklch(0.15_0.01_280)]">
                  ...
                  <div className="flex items-start justify-between gap-2">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        {event.type === "task" && (
                          <Checkbox
                            ...
                            onCheckedChange={(checked) => handleTaskToggle(event.id, !!checked)}
                          />
                        )}
                        <h3
                      ...
                      <div className="flex items-center gap-4 text-xs text-muted-foreground">
                        <span className={badgeClassFor(event.type)}>{getEventTypeLabel(event.type)}</span>
                        <span>
