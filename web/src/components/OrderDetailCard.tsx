  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-4 md:p-6">
      ...
      <div className="mb-6 flex items-center justify-between">
        <Link href="/orders" className="inline-flex items-center gap-2 text-slate-400 hover:text-teal-400 transition-colors">
          <ArrowLeft size={20} />
          <span className="text-sm font-medium">Volver a pedidos</span>
        </Link>
        <div className="flex gap-2">
          <Link href={`/orders/${order.id}/edit`} className="inline-flex items-center gap-2 px-4 py-2 bg-teal-600 hover:bg-teal-700 text-white rounded-lg transition-colors">
            <Pencil size={16} />
          ...
          <div className="bg-gradient-to-br from-slate-800 to-slate-700 rounded-xl p-6 border border-slate-700 shadow-xl">
            <div className="flex items-start justify-between mb-6">
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm text-slate-400">Pedido creado {createdDate}</p>
              </div>
            ...
            <div className="space-y-3 pt-6 border-t border-slate-600">
              <div className="flex items-center gap-3">
                <User size={18} className="text-teal-400 flex-shrink-0" />
                <div className="flex-1 min-w-0">
                  <p className="text-xs text-slate-400">Cliente</p>
                  <Link href={`/clients/${order.cliente_id}`} className="text-sm text-teal-400 hover:text-teal-300 transition-colors truncate">
              ...
              <div className="flex items-center gap-3">
                <Calendar size={18} className="text-teal-400 flex-shrink-0" />
                <div>
                  <p className="text-xs text-slate-400">Fecha del pedido</p>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-xs text-slate-400">Total de items</p>
            ...
            <div className="p-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-3">
                    <div className="p-4 bg-slate-700/50 rounded-lg border border-slate-600">
                      <p className="text-xs text-slate-400 mb-1">ID del pedido</p>
                      <p className="text-sm font-mono text-teal-400">{order.id}</p>
                    </div>
                    <div className="p-4 bg-slate-700/50 rounded-lg border border-slate-600">
                      <p className="text-xs text-slate-400 mb-1">Cliente</p>
                      <Link href={`/clients/${order.cliente_id}`} className="text-sm text-teal-400 hover:text-teal-300 transition-colors">
                        {order.cliente_nombre || 'N/D'} →
                      </Link>
                    </div>
                    <div className="p-4 bg-slate-700/50 rounded-lg border border-slate-600">
              ...
              {activeTab === 'items' && (
                <div className="space-y-4">
                  ...
                  {items.length === 0 ? (
                    <p className="text-slate-400 text-center py-8">No hay items en este pedido</p>
                  ) : (
                    <div className="space-y-3">
                      {items.map((item) => (
                        <div key={item.id} className="p-4 bg-slate-700/50 rounded-lg border border-slate-600 hover:border-teal-500 transition-colors">
                          <div className="flex items-start justify-between mb-2">
                            ...
                            <span className="text-lg font-bold text-teal-400">${item.total_linea.toFixed(2)}</span>
                          </div>
                          <div className="w-full bg-slate-600 rounded-full h-1.5">
                            <div className="bg-gradient-to-r from-teal-400 to-teal-600 h-1.5 rounded-full" style={{ width: `${(item.total_linea / totalAmount) * 100}%` }} />
                          </div>
                        </div>
                      ))}
                    </div>
              ...
              {activeTab === 'notas' && (
                <div className="space-y-4">
                  ...
                  <div className="p-4 bg-slate-700/50 rounded-lg border border-slate-600 border-dashed">
                    <p className="text-slate-400 text-center text-sm">No hay notas registradas. Agrega notas para recordar detalles importantes sobre este pedido.</p>
                  </div>
                </div>
              )}
            </div>
